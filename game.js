/**
 * Protocol: Magic
 * Core Game Logic - Production Ready V1.1 (Stable)
 */
...
const SECTOR_CONFIG = {
    1: { bgTop: '#050011', bgBot: '#100020', sun: ['#ffe600', '#ff0055'], grid: '#00f3ff33' }, // Neon City
    2: { bgTop: '#001115', bgBot: '#002025', sun: ['#ffffff', '#00f3ff'], grid: '#ffffff33' }, // Server Farm
    3: { bgTop: '#150500', bgBot: '#250a00', sun: ['#ff8800', '#ff0000'], grid: '#ff440033' }  // The Core
};

const STATE = {
    BOOT: 0, MENU: 1, MAP: 2, COMBAT: 3, REWARD: 4, GAMEOVER: 6, TUTORIAL: 7, META: 8, SHOP: 9, CHAR_SELECT: 10, EVENT: 11,
    INTEL: 12, HEX: 13,
    TUTORIAL_COMBAT: 14, STORY: 15 // NEW STATES
};
...
const TUTORIAL_PAGES = [
    {
        ...
        content: `
            <p>The year is 21XX. Technology has consumed the Earth. The Silicon Empire controls everything with cold precision.</p>
    ...
    },
    {
        ...
        content: `
            <p><strong>Encrypted Files:</strong> High-ranking machine units (Bosses & Elites) carry encrypted data regarding the fall of humanity.</p>
    ...
    },
    {
        ...
        content: `
            <p>Combat is turn-based. You start with a set of <strong>Dice</strong>.</p>
    ...
    },
    {
        ...
        content: `
            <p>Timing is everything in the digital realm.</p>
    ...
    },
    {
        title: "PROGRESSION",
        content: `
            <p>Defeated enemies drop <strong>Tech Fragments</strong>.</p>
...
const POST_TUTORIAL_PAGES = [
    {
        ...
        content: `
            <p>Your training is complete. You are now connected to the Sector Map.</p>
            <ul style="list-style-type: none; padding: 0;">
    ...
    },
    {
        ...
        content: `
            <p><strong>Base Modules (Unupgraded):</strong></p>
            <ul style="list-style-type: none; padding: 0;">
    ...
    },
    {
        ...
        content: `
            <p>Unlock these via Sanctuary or Events:</p>
    ...
    },
    {
        ...
        content: `
            <p><strong>Elite Unit Modifiers:</strong></p>
            <ul style="list-style-type: none; padding: 0;">
    ...
    },
    {
        ...
        content: `
            <p><strong>💾 Data Files:</strong> Encrypted lore dropped by Bosses. Decrypt them in the Intel menu.</p>
    ...
    },
    {
        title: "MISSION START",
        content: `
            <p>The Silicon Empire has paved the oceans.<br>
...
const UPGRADES_POOL = [
    ...
    // NEW 10 UPGRADES + New Die Unlock
    ...
    { id: 'emergency_kit', name: "Emergency Kit", desc: "Heal 15 HP if you drop below 30% HP (Once/Combat).", icon: "⛑️" },
...
const DICE_TYPES = {
    ...
    EARTHQUAKE: { icon: '📉', color: '#ff8800', desc: 'Deal 5 DMG to ALL enemies.', cost: 2, isSkill: true, target: 'all_enemies' },
    ...
    VOODOO:     { icon: '☠️', color: '#ff0000', desc: 'Deal 100 DMG after 3 turns.', cost: 9, isSkill: true, locked: true, target: 'enemy' },
...
const DICE_UPGRADES = {
    ATTACK:     { name: "Blade Storm", desc: "Deal 8 DMG. 25% chance to hit ALL enemies.", cost: 135, icon: "⚔️" },
    DEFEND:     { name: "Aegis Field", desc: "Gain 8 Shield. All allies gain 3 Shield.", cost: 135, icon: "🏰" },
    MANA:       { name: "Soul Battery", desc: "Gain 2 Mana and Heal 2 HP.", cost: 157, icon: "🔋" },
    // CHANGED: Alpha Call Buff
    MINION:     { name: "Alpha Call", desc: "Summon Level 2 Wisp.\n(+5 Block, +5 DMG)", cost: 172, icon: "🌳" },
    EARTHQUAKE: { name: "Cataclysm", desc: "Deal 8 DMG to ALL. Apply WEAK (50% less dmg).", cost: 225, icon: "🌋" },
    METEOR:     { name: "Starfall", desc: "Deal 50 DMG. [QTE]: Crit x1.3.", cost: 300, icon: "🌠" },
    CONSTRICT:  { name: "Digital Rot", desc: "Reduce Atk/Heal by 75% for 3 turns.", cost: 240, icon: "🕸️" },
    VOODOO:     { name: "Void Curse", desc: "After 3 turns: 50% chance for 500 DMG, else 100.", cost: 337, icon: "🕳️" },
    OVERCHARGE: { name: "Hyper Beam", desc: "Enemy takes +100% Damage from all sources.", cost: 247, icon: "☢️" },
    
    // NEW UPGRADE
    RECKLESS_CHARGE: { name: "Vicious Charge", desc: "Next Attack x3 DMG.\nTake +50% DMG until next turn.", cost: 450, icon: "👹" }
};
...
class Entity {
    ...
    addEffect(id, duration, val, icon, desc, displayName = null) {
        ...
        if (existing) {
            // Refresh duration to the new application's duration
            existing.duration = Math.max(existing.duration, duration);
            
            // Update display name if it changed (e.g. upgrading Constrict to Digital Rot)
            if (displayName) existing.name = name; 
            
            if (id === 'constrict') {
                // FIX & STACKING: Multiply the reduction factors for stronger effect
                // e.g. 0.5 (Constrict) * 0.25 (Rot) = 0.125 (12.5% healing allowed)
                existing.val = existing.val * val;
                
                // Visual confirmation of stack
                ParticleSys.createFloatingText(this.x, this.y - 120, "EFFECT STACKED", "#ff00ff");
            } 
            else if (id === 'weak') {
                 // Weakness logic: Keep the strongest reduction
                 if (val < existing.val) existing.val = val;
            } else {
                 // Standard logic: Keep highest value (e.g. Poison damage)
                 if (val > existing.val) existing.val = val; 
            }
        } else {
...
class Enemy extends Entity {
    ...
    getEffectiveDamage() {
        ...
        // 1. Apply Constrict / Digital Rot
        const constrict = this.hasEffect('constrict');
        if (constrict) {
            dmg = Math.floor(dmg * constrict.val);
        }
        
        // 2. Apply Weak (from Cataclysm) - Stacks multiplicatively
    ...
    updateIntentValues() {
        ...
        // 1. Update Attack Damage
        if (this.nextIntent.type === 'attack') {
            this.nextIntent.val = this.getEffectiveDamage();
        } 
        // 2. Update Heal Amount (NEW: Check for Constrict/Digital Rot)
        else if (this.nextIntent.type === 'heal') {
            // Recalculate base heal (10% of Max HP) to ensure we don't double-apply reductions
...
const Game = {
    ...
    bindEvents() {
        ...
        // HELPER: Robust Mobile/Desktop Click Handler
        const attachButtonEvent = (id, callback) => {
            const btn = d.getElementById(id);
            if (!btn) return;

            // 1. Handle Touch (Mobile)
            // Trigger on 'touchend' for a natural feel, preventing the ghost click
            btn.addEventListener('touchend', (e) => {
                e.stopPropagation();
                e.preventDefault(); // Prevents the browser from firing a subsequent 'click'
                btn.blur(); 
                TooltipMgr.hide();
        ...
        attachButtonEvent('btn-back-tutorial', () => {
            // Reset to default help text when leaving
            this.tutorialData = TUTORIAL_PAGES;
            this.changeState(STATE.MENU); 
        });
        ...
        if (relicBtn) {
            // We use the helper for consistency, but handle the logic here
            relicBtn.addEventListener('touchend', (e) => {
                e.stopPropagation();
                e.preventDefault();
                relicBtn.blur();
                AudioMgr.playSound('click');
        ...
        // Global Click to close dropdowns/Advance Tutorial
        // Note: touchstart is often better for "Click Anywhere" on mobile
        const handleGlobalClick = (e) => {
            ...
            // 2. TUTORIAL LOGIC (Click Anywhere)
            if (this.currentState === STATE.TUTORIAL_COMBAT) {
                ...
                // Pass-through for Step 1 (Click Enemy)
                if (this.tutorialStep === 1 && e.target.id === 'tutorial-overlay') {
                     handleInteraction(e);
                }
            }
        };
        ...
        // Also allow tapping overlay to work
        d.getElementById('tutorial-overlay').addEventListener('touchstart', (e) => {
            if(this.currentState === STATE.TUTORIAL_COMBAT) handleGlobalClick(e);
        });
        ...
        this.canvas.addEventListener('touchstart', (e) => {
            // Don't prevent default here, or we lose scrolling. 
    ...
    startDrag(e, die, el) {
        ...
        if (this.dragState.active) {
            ...
            if (this.dragState.dieElement) this.dragState.dieElement.style.opacity = '1';
        }
    ...
    checkQTE() {
        ...
        if (this.qte.type === 'ATTACK') {
             if (diff < 15) quality = 'perfect'; 
        } else {
             if (diff < 8) quality = 'perfect'; 
             else if (diff < 35) quality = 'good';
        }
        
        if (quality !== 'fail') {
             this.resolveQTE(quality);
        } else {
             if (diff > 50) this.resolveQTE('fail');
        }
    },
    ...
    visitNode(node) {
        ...
        // If clicking the start flag (which is already completed), do nothing
        ...
        // 1. Handle Previous Node Logic
        // Mark previous node completed (visual only, logic is handled by completeCurrentNode)
        if (this.map.currentIdx !== node.id) {
            const previous = this.map.nodes.find(n => n.id === this.map.currentIdx);
            if(previous) previous.status = 'completed';
            
            // Lock siblings (Prevent multi-pathing)
        ...
        // 3. Save Game (Saves at start of encounter)
        this.saveGame();
    ...
    startEvent() {
        ...
        event.options.forEach(opt => {
            ...
            btn.onclick = () => {
                ...
                // Complete node logic
                const node = this.currentEventNode;
                const current = this.map.nodes.find(n => n.id === this.map.currentIdx);
    ...
    renderShop() {
        ...
        this.shopInventory.forEach(item => {
            ...
            if (item.purchased) {
                ...
                div.innerHTML = `<div style="font-size:2rem">✅</div><div class="neon-text-green">ACQUIRED</div>`;
                grid.appendChild(div);
                return;
            }
            ...
            if (item.type === 'item') {
                ...
                div.innerHTML = `<div style="font-size:2rem">${item.icon}</div>
                                 <div class="neon-text-blue">${item.name}</div>
                ...
                div.innerHTML = `<div style="font-size:2rem; position:relative;">
                                    ${item.icon}
                                    ${item.isDiscount ? '<span style="position:absolute; top:-10px; right:-10px; font-size:1rem;">🏷️</span>' : ''}
                                 </div>
                                 <div class="neon-text-gold">${item.name}</div>
                                 <div style="font-size: 0.75rem; color: #aaa; margin-bottom: 2px;">Upgrades: <span style="color:#fff">${item.key}</span></div>
                                 <div class="shop-desc">${item.desc}</div>
                                 <div class="cost-tag">${item.cost} Frags</div>`;
	...
	// NEW: Static Field Relic
        ...
        if(this.enemy) {
             ...
             if (this.enemy.affixes && this.enemy.affixes.includes('Shielded')) {
                 const regen = Math.floor(this.enemy.maxHp * 0.2);
        ...
        this.dicePool.forEach(die => {
            ...
            if(!die.used) {
                ...
                if (['ATTACK', 'EARTHQUAKE', 'METEOR'].includes(die.type)) {
                    ...
                    const finalDmg = this.calculateCardDamage(base);
                    desc = desc.replace(/Deal (\d+) (damage|DMG)/i, `Deal ${finalDmg} $2`);
                }

                // PC: Hover to see tooltip
                el.onmouseenter = (e) => TooltipMgr.show(desc, e.clientX, e.clientY);
                el.onmouseleave = () => TooltipMgr.hide();

                // MOBILE: Touch to see tooltip immediately
                el.addEventListener('touchstart', (e) => {
                    // Get touch coordinates
                    const touch = e.touches[0];
                    // Show tooltip slightly above the finger so it isn't covered
    ...
    useDie(die, el, target) {
        ...
        // FIX: Update UI immediately so the die turns gray/inactive instantly
        this.renderDiceUI(); 
        ...
        const executeAction = (qteMultiplier = 1.0) => {
            ...
            if (this.currentState === STATE.TUTORIAL_COMBAT) {
                ...
                // Victory Check (FIXED: Only check if we are actually ATTACKING)
                if (type === 'ATTACK' && finalEnemy.currentHp - (5 * qteMultiplier) <= 0) {
            ...
            if (type === 'ATTACK' && this.enemy) {
                ...
                dmg = this.calculateCardDamage(dmg); 
                dmg = Math.floor(dmg * qteMultiplier * chargeMult); // Add chargeMult
                ...
                if (target instanceof Minion && target.isPlayerSide) {
                     // Alpha Call Upgrade on existing minion
                    if (isUpgraded) {
                ...
                AudioMgr.playSound('upgrade');
                 ...
                 // Pass "DIGITAL ROT" if upgraded
                 const name = isUpgraded ? "DIGITAL ROT" : "CONSTRICT";
                 finalEnemy.addEffect('constrict', dur, val, '⛓️', 'Atk/Heal reduced.', name);
                 
                 this.performAttackEffect(this.player, finalEnemy);
                 AudioMgr.playSound('attack');
            } else if (type === 'VOODOO') {
                 let val = isUpgraded ? 0 : 100; 
                 if(!isUpgraded) val = Math.floor(val * dmgMult); 
                 
                 // Pass "VOID CURSE" name
                 const name = isUpgraded ? "VOID CURSE" : "VOODOO";
                 finalEnemy.addEffect('voodoo', 3, val, '☠️', 'Doom incoming.', name);
                 ParticleSys.createFloatingText(finalEnemy.x, finalEnemy.y - 120, "CURSED", "#f00");
                 AudioMgr.playSound('attack');
            } else if (type === 'OVERCHARGE') {
                 const val = isUpgraded ? 1 : 0;
                 // Pass "HYPER BEAM" name
                 const name = isUpgraded ? "HYPER BEAM" : "OVERCHARGE";
                 finalEnemy.addEffect('overcharge', 3, val, '⚡', 'Unstable: Dmg Taken increased.', name);
...
// --- TUTORIAL SYSTEM ---
    ...
    startTutorial() {
        ...
        // FIX: Explicitly set the target to the player so the line knows where to draw
        this.enemy.nextIntent = { type: 'attack', val: 5, target: this.player }; 
        
        // Start hidden so the player has to "Scan" it in Step 1
    ...
    updateTutorialStep() {
        ...
        switch(this.tutorialStep) {
            ...
            case 3: // Attack QTE
                ...
                text.innerHTML = "ACTION COMMAND DETECTED.<br><strong>CLICK</strong> inside the ring to deal Critical Damage!";
                break;
