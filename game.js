...
const POST_TUTORIAL_PAGES = [
...

	{
        ...
        content: `
            <p><strong>💡 Tooltips:</strong> If you are ever unsure what an icon, enemy, or skill does, <strong>TAP</strong> (or hover) on it to see details.</p>
    ...
    },
    {
        ...
        options: [
            { 
                ...
                effect: (g) => { 
                    g.startCombat('elite'); 
                    return "COMBAT_STARTED"; // Special Flag
                } 
            },
...
const Game = {
    ...
    useDie(die, el, target) {
        ...
        const executeAction = (qteMultiplier = 1.0) => {
            ...
            // 1. Calculate Damage using current state (BEFORE resetting charge)
            let attackDmg = 0;
            if (type === 'ATTACK') attackDmg = isUpgraded ? 8 : 5;
            else if (type === 'METEOR') attackDmg = isUpgraded ? 50 : 30;
            
            if (attackDmg > 0) {
                attackDmg = this.calculateCardDamage(attackDmg, type);
                attackDmg = Math.floor(attackDmg * qteMultiplier);
            }

            // 2. Reset Charge Multiplier if used
            ...
            if (type === 'ATTACK' && this.enemy) {
                if (type === 'ATTACK') {
                    ...
                    if (triggerRelentless) {
                        attackDmg *= 3.0; // Relentless is a separate multiplier
                ...
                if (isUpgraded && Math.random() < 0.25) {
                    ...
                    if (finalEnemy.takeDamage(attackDmg)) { 
                ...
                this.performAttackEffect(this.player, finalEnemy);
                // Damage calculated at start of block
                ...
                if (finalEnemy.takeDamage(attackDmg)) {
                 ...
                 // FIX: Use correct Name and Icon based on upgrade status
