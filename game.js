...
const Game = {
    ...
    bindEvents() {
        ...
        // NEW: Settings Logic with Tutorial Lock
        const toggleSettings = () => {
            if (this.currentState === STATE.TUTORIAL_COMBAT) {
                // Calculate center of screen for text
                // Using fixed center because canvas rect might be behind overlay
                const x = window.innerWidth / 2;
                const y = window.innerHeight / 2;
                
                // Show warning text above everything (z-index handled by ParticleSys if adjusted, 
                // but ParticleSys uses canvas which might be highlighted or not. 
                // Safe bet: Use Audio feedback + ParticleText)
                
                // Note: Floating text draws to canvas. 
                // If tutorial canvas is highlighted (z-index 2000), text shows.
                // If not, it might be behind overlay.
                // However, Audio is the key feedback.
                
                ParticleSys.createFloatingText(CONFIG.CANVAS_WIDTH/2, CONFIG.CANVAS_HEIGHT/2, "COMPLETE TUTORIAL TO UNLOCK SETTINGS", "#ff0000");
                AudioMgr.playSound('hit'); 
                return;
            }
        ...
        attachButtonEvent('btn-settings', toggleSettings);
        attachButtonEvent('btn-settings-main', toggleSettings);
    ...
    visitNode(node) {
        ...
        if(node.type === 'combat' || node.type === 'elite' || node.type === 'boss') {
            ...
            // NEW: Update Tinker Button based on availability
