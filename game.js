...
const Game = {
    ...
    startTutorial() {
        ...
        // Setup Mock Player & Enemy (HP increased for longer tutorial)
        this.player = new Player(PLAYER_CLASSES[0]); 
        this.player.currentHp = 100; // Increased HP
        this.player.maxHp = 100;
        this.player.mana = 3;
        this.player.diceCount = 2; 
        
        // Enemy HP increased to survive multiple steps (Attack/Defend cycle)
        this.enemy = new Enemy({ name: "Training Dummy", hp: 100, dmg: 5 }, 1);
     ...
     updateTutorialStep() {
        ...
        // Remove previous click handlers to prevent immediate advancement
        document.getElementById('hud').onclick = null;
        document.getElementById('gameCanvas').onclick = null;
        
        // Helper function to advance step on focus element click
        const advanceOnElementClick = (elementId) => {
             const el = document.getElementById(elementId);
             if (el) {
                 el.classList.add('tutorial-focus');
                 el.onclick = () => {
                     el.onclick = null; // Remove handler after one click
                     this.tutorialStep++;
                     this.updateTutorialStep();
                 };
             }
        ...
        let topPercent = '40%'; // Default position (Original Position)
        const isMobile = window.innerWidth <= 768;

        if (isMobile) {
            // Your specified positions: 40% (Upper) vs 75% (Lower)
            switch (this.tutorialStep) {
                case 0: // Intro (Original: 40%)
                case 4: // Drag Defend Die (Original: 40%)
                case 5: // End Phase (Original: 40%)
                case 6: // QTE Defend (Original: 40%)
                case 8: // Summon Die (Original: 40%)
                case 9: // Destroy Target (Original: 40%)
                case 11: // Summon (Actual step)
                case 12: // Final Attack
                    topPercent = '40%'; 
                    break;
                case 1: // Player Stats (New Lower: 75%)
                case 2: // Enemy/Intent (New Lower: 75%)
                case 3: // Dice Overview (New Lower: 75%)
                case 7: // Reroll Button (New Lower: 75%)
        ...
        switch(this.tutorialStep) {
            case 0: 
                text.innerHTML = "SIMULATION BOOT.<br>Welcome to Protocol: Magic.<br>Your objective is to infiltrate the core.<br><br><strong>[CLICK ANYWHERE TO BEGIN]</strong>";
                // Advance tutorial on *any* non-dice/button click
            ...
            case 1: 
                text.innerHTML = "OPERATOR STATS: This is your **Health Bar**. Below it is your **Mana (3/3)**. You start with 3 Mana and gain 1 each turn. Mana is used for powerful Skill Dice.<br><strong>[TAP YOUR HP BAR TO FOCUS]</strong>";
                // Focus: HP Bar. No automatic advance. Advance on clicking the HP Bar element (which is part of the HUD, not the canvas).
                const hpBarEl = document.querySelector('#hud .resources');
                if (hpBarEl) hpBarEl.classList.add('tutorial-focus');
                
                // Special: Bind event to advance tutorial from clicking the focused HP bar area
                canvas.onclick = () => {
                    // Check if click is near player (a proxy for the invisible HP bar area over the canvas)
                    const dist = Math.hypot(this.mouseX - this.player.x, this.mouseY - this.player.y);
            ...
            case 2:
                text.innerHTML = "ENEMY THREAT: This is the **Enemy Health Bar**. The icon above them shows their **Intent** (what they will do on their turn).<br>Click or hold the Enemy to get a detailed combat prognosis.";
                // Focus: Canvas around Enemy. Advance happens when the enemy.showIntent property is toggled in the canvas mouse handler.
                canvas.classList.add('tutorial-focus');
                break;

            case 3:
                text.innerHTML = "MODULES: The bottom bar holds your **Dice Modules**. You can re-roll any un-used or selected module, then **Drag & Drop** it onto a valid target (Enemy or Self).";
                // Focus: Dice/Control Bar Area
                document.getElementById('dice-container').classList.add('tutorial-focus');
            ...
            case 4: 
                text.innerHTML = "KNOWLEDGE IS POWER: Click the Enemy Unit to scan their Intent and Status.";
                // Focus: Enemy Unit Area
                canvas.classList.add('tutorial-focus');
                break;

            case 5: 
                text.innerHTML = "Enemy intent detected. DRAG the **Attack Module** onto the enemy unit. This uses 0 Mana.";
                // Focus: Attack die and enemy
                const dice5 = document.querySelectorAll('#dice-container .die');
                if(dice5[0]) dice5[0].classList.add('tutorial-focus'); 
                canvas.classList.add('tutorial-focus'); 
                break;
            ...
            case 6: 
                ...
                text.innerHTML = "ACTION COMMAND: Click inside the inner ring during the attack to achieve a **Critical Hit** (+30% DMG)!";
                break;
            ...
            case 7: 
                ...
                text.innerHTML = `
                    ...
                    Incoming damage predicted. DRAG the **Shield Module** to your avatar (Self-Target).
                `;
                // Focus: Defend die and player
                const dice7 = document.querySelectorAll('#dice-container .die');
                if(dice7[1]) dice7[1].classList.add('tutorial-focus');
                canvas.classList.add('tutorial-focus');
                break;

            case 8: 
                text.innerHTML = "Cycle complete. TAP the 'END PHASE' button. The Enemy will execute their Intent now.";
                // Focus: End Turn Button. Advance on click.
                advanceOnElementClick('btn-end-turn');
                break;
            ...
            case 10: 
                text.innerHTML = "MODULES EXHAUSTED: You get 2 free re-rolls per turn. TAP the **Reroll icon** to generate new data for the selected modules.";
                // Focus: Reroll Button. Advance on click.
                advanceOnElementClick('btn-reroll');
                this.rollDice(2); 
                break;

            case 11: 
                text.innerHTML = "REINFORCEMENTS: DRAG the **Minion/Wisp Module** onto empty canvas space. Your Wisp will attack automatically at the end of your turn.";
                // Focus: Minion Die and Canvas area.
                setTimeout(() => {
            ...
            case 12: 
                // Final step - Use any remaining dice to win.
