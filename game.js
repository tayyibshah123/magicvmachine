...
const Game = {
    ...
    generateMap() {
        ...
        // 1. Create Start Node
        this.map.nodes.push({ id: 'start', layer: 0, lane: -1, x: 50, y: 95, type: 'start', connections: [], status: 'available' });
        ...
        for(let l=1; l<=levels; l++) {
            ...
            for(let i=0; i<nodeCount; i++) {
                ...
                if(nodeCount === 3) {
                    // Base positions: 20%, 50%, 80%
                    x = 20 + (i * 30);
                    
                    // NEW: Zig-Zag Offset
                    // Even layers shift left (-4%), Odd layers shift right (+4%)
                    // This prevents vertical stacking overlap
                    const zigZag = (l % 2 === 0) ? -4 : 4;
                    x += zigZag;
                }
                
                // Reduced jitter to keep the zig-zag pattern clean
