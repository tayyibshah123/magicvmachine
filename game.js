...
const Game = {
    ...
    updateTutorialStep() {
        ...
        if (isMobile) {
            // Your specified positions: 40% (Original) vs 75% (New Lower)
            switch (this.tutorialStep) {
                ...
                case 9: // Destroy Target (Original: 40%)
                    topPercent = '40%'; 
                    break;
                case 1: // Enemy Scan (New Lower: 75%)
                case 2: // Drag Attack Die (New Lower: 75%)
                case 3: // QTE Attack (New Lower: 75%)
        ...
        text.style.transform = 'translateX(-50%) translateY(-50%)'; // Ensure centering remains correct

        // ----------------------------------------------------

        switch(this.tutorialStep) {
            case 0: 
                text.innerHTML = "SIMULATION INITIALIZED.<br>Welcome, Operator. We must verify your interface connection.<br><br><strong>[CLICK ANYWHERE TO CONTINUE]</strong>";
                break;

            case 1: 
                // FIX: Removed internal line breaks that caused poor layout
                text.innerHTML = "Knowledge is power. CLICK OR HOLD the Enemy Unit to scan their Status.";
                canvas.classList.add('tutorial-focus'); 
                break;

            case 2: 
                text.innerHTML = "Enemy intent detected. <strong>DRAG</strong> the Attack Module onto the target.";
            ...
            case 3: 
                ...
                text.innerHTML = "ACTION COMMAND DETECTED. <strong>CLICK</strong> inside the inner ring to achieve a Critical Hit!";
                break;

            case 4: 
                overlay.classList.remove('hidden');
                text.innerHTML = `
                    Excellent. Critical Hit registered. 
                    ...
                    Incoming damage predicted. <strong>DRAG</strong> the Shield Module to your avatar.
                `;
                setTimeout(() => {
                    const dice = document.querySelectorAll('#dice-container .die');
            ...
            case 5: 
                text.innerHTML = "Cycle complete. <strong>TAP</strong> 'END PHASE' to proceed.";
                document.getElementById('btn-end-turn').classList.add('tutorial-focus');
                break;

            case 6: 
                overlay.classList.add('hidden');
                text.innerHTML = "INCOMING ATTACK. <strong>CLICK</strong> the ring on your avatar to BLOCK!";
                break;

            case 7: 
                text.innerHTML = "Modules exhausted. <strong>TAP</strong> the Reroll icon to generate new data.";
                document.getElementById('btn-reroll').classList.add('tutorial-focus');
                break;

            case 8: 
                text.innerHTML = "Reinforcements available. <strong>DRAG</strong> the Summon Module to empty space.<br><span style='font-size:0.8rem; color:#aaa'>(Minions deal damage and can take hits for you.)</span>";
                setTimeout(() => {
                    const dice = document.querySelectorAll('#dice-container .die');
                    if(dice[0]) dice[0].classList.add('tutorial-focus');
                    canvas.classList.add('tutorial-focus');
            ...
            case 9: 
                // All steps now use the transparent style from CSS, no need for the extra class here.
                text.innerHTML = "DESTROY the target.";
                overlay.classList.add('hidden');
                setTimeout(() => {
                    const dice = document.querySelectorAll('#dice-container .die');
                    if(dice[1]) dice[1].classList.add('tutorial-focus');
