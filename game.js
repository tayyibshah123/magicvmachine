...
const Game = {
    ...
    drawHealthBar(entity) {
        ...
        if (entity instanceof Player) {
            const mx = x - 15; // Padding from left of bar
            const my = y + height/2;
            
            // Draw Value first (Right aligned to the padding)
            ctx.font = 'bold 22px "Orbitron"';
            ctx.textAlign = 'right';
            ctx.textBaseline = 'middle';
            ctx.fillStyle = '#fff';
            ...
            // Draw Icon to the left of the value
            // Calculate text width to position icon dynamically, or use fixed offset
            // Fixed offset is usually cleaner for single digits
            ctx.font = '24px Arial';
            ctx.fillStyle = COLORS.MANA;
            ctx.fillText("💠", mx - 25, my); 
        }
