...
const Game = {
    ...
    updateTutorialStep() {
        ...
        // --- DYNAMIC POSITIONING LOGIC (Mobile-friendly) ---
        let topPercent = '40%'; // Default/Desktop position
        let textClass = '';

        if (window.innerWidth <= 768) {
            // Mobile-specific layout decisions (using percentage of viewport height)
            switch (this.tutorialStep) {
                case 0: // Intro
                case 1: // Enemy Scan
                    topPercent = '45%'; // Middle-upper to clear both entities
                    break;
                case 2: // Drag Attack Die
                case 3: // QTE Attack
                case 4: // Drag Defend Die
                    topPercent = '20%'; // High up to clear everything in the lower half
                    break;
                case 5: // End Phase
                case 6: // QTE Defend
                ...
                case 9: // Destroy Target
                    topPercent = '70%'; // Low down to clear entities and highlight bottom HUD
                    break;
            }
            // Apply dynamic position for mobile
            text.style.top = topPercent;
            text.style.transform = 'translateX(-50%) translateY(-50%)'; // Ensure centering remains correct
        } else {
            // Apply default desktop position
            text.style.top = '40%';
