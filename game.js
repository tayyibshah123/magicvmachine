...
class Entity {
    ...
    takeDamage(amount) {
        ...
        // Apply Player Incoming Damage Multiplier (Reckless Charge)
        ...
        // 2. Firewall Relic (Cap FIRST unblocked damage to HP)
        if (this instanceof Player && this.hasRelic('firewall') && !this.firewallTriggered && actualDmg > 0) {
            ...
            // Calculate Cap: 1 stack = 50, 2 = 40, 3 = 30
        ...
        if(this instanceof Player && actualDmg > 0) {
            Game.shake(5);
            if(this.hasRelic('spike_armor') && Game.enemy) {
                let spikeDmg = 3;
                const spikes = this.relics.filter(r => r.id === 'spike_armor').length;
                spikeDmg *= spikes;
                
                Game.enemy.takeDamage(spikeDmg);
    ...
    heal(amount) {
        ...
        if (constrict) {
            const reduction = constrict.val > 0.5 ? 0.25 : 0.5;
            actualHeal = Math.floor(actualHeal * reduction);
    ...
    addEffect(id, duration, val, icon, desc, displayName = null) {
        ...
        // If name provided, use it, otherwise default to ID
        const name = displayName || id.toUpperCase();
        
        if (existing) {
            existing.duration = duration;
            // Update display name if it changed (e.g. applying upgrade over base)
            if (displayName) existing.name = displayName; 
            // Keep the stronger effect value (lower multiplier is stronger for reduction)
            if (id === 'constrict' || id === 'weak') {
...
const Game = {
    ...
    bindEvents() {
        ...
        // This fixes the "Stuck Button" and "Invalid Target" issues by stopping propagation and blurring focus
        const attachButtonEvent = (id, callback) => {
            ...
            btn.onclick = (e) => {
                e.stopPropagation(); 
                e.preventDefault();  
                btn.blur(); // Removes focus/active state
                TooltipMgr.hide();
        ...
        attachButtonEvent('btn-debrief', () => this.changeState(STATE.TUTORIAL));
        attachButtonEvent('btn-back-tutorial', () => this.changeState(STATE.MENU));
        ...
        attachButtonEvent('btn-end-turn', () => {
            // Clear dice selection to prevent visual bugs
        ...
        const handleInteraction = (e) => {
             ...
             // Prevent intent toggle if we are dragging a die
             if (this.dragState.active) return;

             if (this.currentState === STATE.COMBAT && this.enemy && this.enemy.currentHp > 0) {
        ...
        // NEW: Handle drag interruptions (e.g. drag off screen, notification, scrolling)
        // This prevents the "Unplayable Die" bug where the game thinks you are still dragging.
        window.addEventListener('pointercancel', (e) => {
            if (this.dragState.active) {
                this.handleDragEnd(e); // Force end the drag cleanly
                if(dragRaf) { cancelAnimationFrame(dragRaf); dragRaf = null; }
            }
        });
    },
    ...
    handleCanvasHover(screenX, screenY) {
        if(this.currentState !== STATE.COMBAT) { TooltipMgr.hide(); return; }
    ...
    updateQTE(dt) {
        ...
        const shrinkSpeed = 160 * dt; 
        this.qte.radius -= shrinkSpeed;
    ...
    renderTutorial() {
        ...
        document.getElementById('tut-page-num').innerText = `${this.tutorialPage + 1} / ${TUTORIAL_PAGES.length}`;
        
        document.getElementById('btn-tut-prev').disabled = (this.tutorialPage === 0);
        document.getElementById('btn-tut-next').disabled = (this.tutorialPage === TUTORIAL_PAGES.length - 1);
        
        document.getElementById('btn-tut-prev').style.opacity = (this.tutorialPage === 0) ? 0.3 : 1;
        document.getElementById('btn-tut-next').style.opacity = (this.tutorialPage === TUTORIAL_PAGES.length - 1) ? 0.3 : 1;
    },
    ...
    generateMap() {
        ...
        // 1. Create Start Node
        this.map.nodes.push({ id: 'start', layer: 0, lane: -1, x: 50, y: 95, type: 'start', connections: [], status: 'completed' }); // Start is completed by default
        ...
        for(let l=1; l<=levels; l++) {
            ...
            for(let i=0; i<nodeCount; i++) {
                ...
                if (l === levels) type = 'boss';
                    ...
                    if (l === 3 || l === 5) pool = pool.filter(t => t !== 'shop');

                    type = pool[Math.floor(r * pool.length)] || 'combat';
                }

                let x = 50;
                if(nodeCount === 3) x = 20 + (i * 30);
                const jitter = (Math.random() - 0.5) * 2;

                // FIX: If Layer 1, set to 'available', otherwise 'locked'
                const initStatus = (l === 1) ? 'available' : 'locked';
		...
		if(this.player.hasRelic('thorn_mail')) this.player.addShield(1);
			...
			// Neural Link Relic
                ...
                targets.forEach(t => {
                    ...
                    if(isUpgraded) {
                        // CHANGED: Explicitly pass "WEAK" as name
                ...
                dmg = this.calculateCardDamage(dmg); 
                dmg = Math.floor(dmg * qteMultiplier); 
                ...
                AudioMgr.playSound('upgrade');
                 ...
                 // CHANGED: Pass "DIGITAL ROT" if upgraded
                 ...
                 finalEnemy.addEffect('voodoo', 3, val, '☠️', 'Cursed: Doom incoming.');
                 ParticleSys.createFloatingText(finalEnemy.x, finalEnemy.y - 120, "CURSED", "#f00");
                 AudioMgr.playSound('attack');
            } else if (type === 'OVERCHARGE') {
                 const val = isUpgraded ? 1 : 0;
                 finalEnemy.addEffect('overcharge', 3, val, '⚡', 'Unstable: Dmg Taken increased.');
    ...
    async endTurn() {
        ...
        for (const m of this.player.minions) {
            ...
            if(t && t.currentHp > 0) {
                ...
                AudioMgr.playSound('attack');
                if (t.takeDamage(m.dmg) && t === this.enemy) { this.winCombat(); return; }
        ...
        if(intent.type === 'attack') {
            ...
            if (validTarget === this.player) {
                ...
                if (validTarget.takeDamage(dmg) && validTarget === this.player) { this.gameOver(); return; }
            } else {
                // Enemy attacks Player Minion
                this.performAttackEffect(this.enemy, validTarget);
                AudioMgr.playSound('attack');
                if (validTarget.takeDamage(intent.val)) {
                     ...
                     if (this.player.traits.maxMinions === 3 && Math.random() < 0.3) { 
                         ...
                         // NEW: Annihilator Bomb Bot Logic
        ...
        for (const min of this.enemy.minions) {
            ...
            if(t) {
                ...
                AudioMgr.playSound('attack');
                if (t.takeDamage(min.dmg) && t === this.player) { this.gameOver(); return; }
    ...
    loop(timestamp) {
        ...
        if (this.currentState === STATE.COMBAT && this.player && this.enemy) {
